    ...
    <main className="min-h-screen">
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/80">
        <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-3 md:px-8">
          ...
          <div className="hidden items-center gap-5 text-sm text-slate-600 dark:text-slate-300 md:flex">
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-5 py-6 md:px-8 md:py-10">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
            <div className="space-y-8">
              <div className="space-y-5">
                <Badge className="eyebrow border-slate-300 bg-slate-100 text-slate-700 shadow-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                  Software Innovator • Deployment Engineer
                </Badge>
                <div className="space-y-5">
                  <h1 className="max-w-4xl text-4xl font-semibold tracking-[-0.05em] text-balance text-slate-950 dark:text-slate-100 sm:text-5xl md:text-7xl">
                  ...
                  <p className="max-w-2xl text-[15px] leading-8 text-slate-600 dark:text-slate-300 md:text-lg">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button
                  asChild
                  size="lg"
                  className="group h-12 w-full rounded-full bg-slate-950 px-6 text-white shadow-[0_18px_40px_-20px_rgba(15,23,42,0.6)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-slate-900 sm:w-auto"
                >
                  <Link href="#projects">
                    View projects
                ...
                <Button
                  ...
                  variant="outline"
                  size="lg"
                  className="h-12 w-full rounded-full border-slate-300 bg-white px-6 text-slate-800 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-400 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700 sm:w-auto"
                >
              ...
              <div className="flex flex-wrap gap-3">
                {highlights.map((item) => (
                  <div
                    key={item}
                    className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white/80 px-4 py-2 text-sm text-slate-600 shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
                  >
            ...
            <Card className="premium-surface relative overflow-hidden border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
              ...
              <CardHeader className="relative pt-10">
                ...
                <div className="flex items-center gap-4 rounded-[1.5rem] border border-slate-200/70 bg-white/92 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/90">
                  <Avatar className="size-18 border-2 border-white shadow-md">
                    <AvatarImage src="/alanmoxlogo.jpeg" alt="Alanmox logo" />
                    <AvatarFallback>AM</AvatarFallback>
                  </Avatar>
                  <div className="space-y-1">
                    <p className="text-xl font-semibold text-slate-950 dark:text-slate-100">Alanmox</p>
              ...
              <CardContent className="relative space-y-6">
                ...
                <div className="grid gap-3 grid-cols-1 sm:grid-cols-3">
                  {metrics.map((metric) => (
                    <div
                      key={metric.label}
                      className="rounded-[1.35rem] border border-slate-200/70 bg-slate-50/90 p-4 dark:border-slate-700 dark:bg-slate-800/80"
                    >
                      <p className="text-lg font-semibold text-slate-950 dark:text-slate-100">{metric.value}</p>
                      <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{metric.label}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    <Badge
                      key={skill}
                      variant="outline"
                      className="rounded-full border-slate-200 bg-slate-50 px-3 py-1.5 text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
                    >
                      {skill}
                    </Badge>
                  ))}
        ...
        <section id="services" className="grid gap-6 md:grid-cols-3">
          {services.map((service) => {
            ...
            return (
              <Card
                key={service.title}
                className="premium-surface group border-slate-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600"
              >
                <CardHeader className="space-y-5 p-5 sm:p-7">
                  ...
                  <div className="space-y-2">
                    <CardTitle className="text-xl text-slate-950 dark:text-slate-100">{service.title}</CardTitle>
                    <CardDescription className="text-[15px] leading-7 text-slate-600 dark:text-slate-300">
                      {service.description}
                    </CardDescription>
                  </div>
                </CardHeader>
        ...
        <section id="projects" className="space-y-6">
          <div className="space-y-3 px-1">
            <Badge className="eyebrow border-amber-200 bg-amber-50 text-slate-700 dark:border-amber-400/40 dark:bg-amber-400/10 dark:text-amber-200">
              Featured Work
            </Badge>
            <h2 className="section-title">Selected projects</h2>
            <p className="max-w-2xl text-[15px] leading-7 text-slate-600 dark:text-slate-300 md:text-base">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {projects.map((project) => (
              <Card
                key={project.title}
                className="premium-surface group overflow-hidden border-slate-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600"
              >
                ...
                <CardHeader className="space-y-4 p-5 pb-4 sm:p-7 sm:pb-4">
                  <div className="flex items-center justify-between gap-3">
                    <Badge className="rounded-full border-0 bg-slate-100 px-3 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-200">
                      {project.label}
                    </Badge>
                    <MoveRight className="size-4 text-slate-400 transition-transform duration-300 group-hover:translate-x-1 dark:text-slate-500" />
                  </div>
                  <div className="space-y-2">
                    <CardTitle className="text-xl text-slate-950 dark:text-slate-100">{project.title}</CardTitle>
                    <CardDescription className="text-[15px] leading-7 text-slate-600 dark:text-slate-300">
                      {project.description}
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent className="flex flex-wrap gap-2 px-5 pb-5 sm:px-7">
                  {project.stack.map((item) => (
                    <Badge
                      key={item}
                      variant="secondary"
                      className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
                    >
                      {item}
                    </Badge>
                  ))}
                ...
                <CardContent className="space-y-3 px-5 pb-5 pt-0 sm:px-7">
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Problem:</span> {project.problem}
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Solution:</span> {project.solution}
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">Impact:</span> {project.impact}
                  </div>
                </CardContent>
                <CardFooter className="px-5 pb-6 pt-0 sm:px-7 sm:pb-7">
                  <Button
                    asChild
                    variant="ghost"
                    className="group/cta h-auto px-0 text-slate-900 hover:bg-transparent dark:text-slate-100"
                  >
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <Card className="premium-surface border-white/85 bg-white/82">
            <CardHeader className="p-5 sm:p-7">
              <Badge className="eyebrow w-fit border-slate-300 bg-slate-100 text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                Expertise
              </Badge>
              <CardTitle className="pt-3 text-2xl text-slate-950 dark:text-slate-100">What I do</CardTitle>
              <CardDescription className="text-[15px] leading-7 text-slate-600 dark:text-slate-300">
                A quick summary of how I support client goals through innovation and delivery.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 p-5 pt-0 text-sm leading-7 text-slate-600 dark:text-slate-300 sm:p-7 sm:pt-0">
              <div className="flex items-start gap-4 rounded-[1.5rem] border border-slate-200/80 bg-slate-50/80 p-5 dark:border-slate-700 dark:bg-slate-800/70">
                <BriefcaseBusiness className="mt-0.5 size-4 text-slate-700 dark:text-slate-300" />
                <p>
              ...
              <div className="flex items-start gap-4 rounded-[1.5rem] border border-slate-200/80 bg-slate-50/80 p-5 dark:border-slate-700 dark:bg-slate-800/70">
                <Code2 className="mt-0.5 size-4 text-slate-700 dark:text-slate-300" />
                <p>
              ...
              <div className="flex items-start gap-4 rounded-[1.5rem] border border-slate-200/80 bg-slate-50/80 p-5 dark:border-slate-700 dark:bg-slate-800/70">
                <Rocket className="mt-0.5 size-4 text-slate-700 dark:text-slate-300" />
                <p>
          ...
          <Card className="premium-surface border-slate-200 bg-slate-50/40">
            <CardHeader className="p-5 sm:p-7">
              <Badge className="eyebrow w-fit border-amber-200 bg-white/80 text-slate-700 dark:border-amber-400/40 dark:bg-amber-400/10 dark:text-amber-200">
                FAQ
              </Badge>
              <CardTitle className="pt-3 text-2xl text-slate-950 dark:text-slate-100">
            ...
            <CardContent className="p-5 pt-0 sm:p-7 sm:pt-0">
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="services" className="border-b border-slate-300/60">
                  <AccordionTrigger className="text-left text-base font-medium text-slate-900 hover:text-blue-700 dark:text-slate-100 dark:hover:text-blue-400">
        ...
        <section id="contact">
          <Card className="relative overflow-hidden rounded-[2rem] border-0 bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_60%,#2563eb_100%)] text-primary-foreground shadow-[0_30px_80px_-30px_rgba(15,23,42,0.55)]">
            ...
            <CardHeader className="relative p-5 sm:p-7 md:p-10">
            ...
            <CardContent className="relative flex flex-col gap-4 p-5 pt-0 sm:p-7 sm:pt-0 md:p-10 md:pt-0 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap gap-3">
                <div className="flex w-full items-center gap-3 rounded-full border border-white/12 bg-white/10 px-4 py-2 text-sm text-white/88 backdrop-blur sm:w-auto">
              ...
              <Button
                ...
                variant="secondary"
                size="lg"
                className="h-12 w-full rounded-full border border-white/10 bg-white px-6 text-slate-900 shadow-[0_18px_35px_-20px_rgba(15,23,42,0.45)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-slate-100 sm:w-auto"
              >
